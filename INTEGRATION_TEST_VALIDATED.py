#!/usr/bin/env python3
"""
FINAL VALIDATED INTEGRATION TEST
Uses actual API signatures verified from the codebase
Tests the QMO system with correct constructor arguments and method names

Run with: python INTEGRATION_TEST_VALIDATED.py
"""
...
def test_02_iceberg_detection():
    ...
    # Test Zone Engine with ACTUAL signature
...
def test_03_liquidity_analysis():
    ...
    from backend.intelligence.iceberg_memory import IcebergMemory
    ...
    # Test Story Engine with ACTUAL signature (requires iceberg_memory)
    iceberg_memory = IcebergMemory()
...
def test_05_signal_filters():
    ...
    # Test Risk Engine with ACTUAL signature (requires account_balance)
...
def test_06_session_management():
    ...
    # Test Position Sizer with ACTUAL signature (uses calculate_lot_size)
    sizer = PositionSizer()
...
def test_08_replay_pipeline():
    ...
    # Test with actual ReplayEngine signature (requires all 7 engines)
    replay = ReplayEngine(
        qmo=MockEngine(),
        imo=MockEngine(),
        gann=MockEngine(),
...
def test_09_end_to_end_integration():
    ...
    # Test Signal Builder with ACTUAL signature
...
def run_all_tests():
    ...
    print("\n" + "="*100)
    print("QUANTUM MARKET OBSERVER - VALIDATED INTEGRATION TEST".center(100))
    print("Using actual API signatures from verified codebase".center(100))
    print("="*100)
    ...
    status = "✅ ALL TESTS PASSED" if failed == 0 else f"⚠️  {failed}/9 TESTS FAILED"
    print(f"\n{status}".center(100))
    ...
    if failures:
        ...
        print("\n🎯 System Validation Complete - All Core Engines Operational")
